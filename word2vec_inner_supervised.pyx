...
def train_sentence_sg_mod(model, sentence, alpha, _work):
    print ' Inside the Mod Cython Function '
    cdef int hs = model.hs
    cdef int negative = model.negative
    ...
    cdef unsigned long long table_len
    cdef unsigned long long next_random
    cdef dict label_dict = model.label_dict
...
def train_sentence_sg_categ(model, sentence, alpha, _work):
    print ' Inside the Cython categ Function '
    cdef int hs = model.hs
    cdef int negative = model.negative
...
def train_sentence_sg_categ_nogil(model, sentence, alpha, _work):
    ...
    cdef np.uint32_t *points[MAX_SENTENCE_LEN]
    cdef np.uint8_t *codes[MAX_SENTENCE_LEN]
    cdef np.uint32_t *labelholder[MAX_SENTENCE_LEN]
...
def train_sentence_sg_categ_my_approach(model, sentence, alpha, _work):
    print ' Inside the Cython categ Function '
    cdef int hs = model.hs
    cdef int negative = model.negative
    ...
    print ' Model negative ', model_label
    ...
    ###### Updating labels with itself // Other approach is pulling the labels of django and scala toward python . . . . . . . . . . .
        ...
        for j in range(j, k):
            ...
            if negative:
                ...
                if indexes[i] in label_holder:
                    label_ = label_holder[indexes[i]]
                    for label_index_ in label_:
                        next_random = fast_sentence_sg_neg(negative, table, table_len, syn0, syn1neg, size, label_index_, indexes[i], _alpha, work, next_random)
