...
# -*- coding: utf-8 -*-
#
# Copyright (C) 2013 Radim Rehurek <me@radimrehurek.com> + 'Sarath R Nair <s4sarath@gmail.com>'
...
try:
    from gensim.models.word2vec_inner_supervised import  train_sentence_sg, train_sentence_sg_categ_nogil, train_sentence_cbow, FAST_VERSION
except Exception as e:
    # failed... fall back to plain numpy (20-80x slower training than the above)
    print " Unable to load the function ( train_sentence_sg_categ_nogil ) due to following reason "
    print "\n"
    print e
    FAST_VERSION = -1
        ...
        for pos, word in enumerate(sentence):
            ...
            for pos2, word2 in enumerate(sentence[start : pos + model.window + 1 - reduced_window], start):
                ...
                if word2 and not (pos2 == pos):
                    if self.label_dict[word]:
                        word = self.label_dict[word]
...
class Word2Vec_Supervised(utils.SaveLoad):
    """
    ...
    def __init__(self, sentences=None, size=100, alpha=0.025, window=5, min_count=5,
        ...
        cbow_mean=0, hashfxn=hash, iter=1, label_dict = None):
        """
        ...
        `label_dict` = a dictionary of word and labels ( list of labels )
        ...
        if sentences is not None:
            ...
            if label_dict is not None:
                self.build_vocab(sentences, label_dict)
                sentences = utils.RepeatCorpusNTimes(sentences, iter)
                self.train(sentences)
            else:
                raise TypeError("Supervised Word2Vec expects a label dictionary")
    ...
    def build_vocab(self, sentences, label_dict):
        """
        ...
        vocab1 = self._vocab_from(sentences)
        label_list = []
        for word_, label_ in iteritems(label_dict):
            label_list.extend(label_)

        vocab2 = self._vocab_from_dict(label_list)    
        vocab = vocab1.copy()
        vocab.update(vocab2)
    ...
    def _vocab_from(sentences):
        ...
        for sentence_no, sentence in enumerate(sentences):
            if sentence_no % 10000 == 0:
                logger.info("Vocab Creation: PROGRESS: at sentence #%i, processed %i words and %i word types" %
    ...
    def _vocab_from_dict(self, sentence):
        ...
        for word in sentence:
            ...
            if word not in vocab:
                ...
                total_words += 1
                vocab[word] = Vocab(count=self.min_count+1)
    ...
    def load_word2vec_format(cls, fname, fvocab=None, binary=False, norm_only=True):
        ...
        with utils.smart_open(fname) as fin:
            ...
            result = Word2Vec_Supervised(size=layer1_size)
    ...
    def most_similar_category(self, positive=[], negative=[], topn=10):
        """
        Find the top-N most similar words. Positive words contribute positively towards the
        similarity, negative words negatively.
        ...
        dists = dot(self.syn1neg_norm, mean)
        if not topn:
            return dists
        best = argsort(dists)[::-1][:topn + len(all_words)]
        # ignore (don't return) words from the input
    ...
    def init_sims(self, replace=False):
        ...
        self.syn1neg_norm = (self.syn1neg / sqrt((self.syn1neg ** 2).sum(-1))[..., newaxis]).astype(REAL)
    ...
    def __str__(self):
        return "Word2Vec_Supervised(vocab=%s, size=%s, alpha=%s)" % (len(self.index2word), self.layer1_size, self.alpha)


    def save(self, *args, **kwargs):
        kwargs['ignore'] = kwargs.get('ignore', ['syn0norm']) # don't bother storing the cached normalized vectors
        super(Word2Vec_Supervised, self).save(*args, **kwargs)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        sys.exit(1)
    infile = sys.argv[1]
    from gensim.models.word2vec_supervised import Word2Vec_Supervised  # avoid referencing __main__ in pickle

    seterr(all='raise')  # don't ignore numpy errors

    # model = Word2Vec(LineSentence(infile), size=200, min_count=5, workers=4)
    model = Word2Vec_Supervised(Text8Corpus(infile), size=200, min_count=5, workers=1)
